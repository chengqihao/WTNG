...
namespace stkq {
    ...
    public:
        ...
        Array<T>& operator= (Array<T>&& p_right); //  定义等于操作符(右引用)

        Array<T>& operator= (const Array<T>& p_right);  // 定义等于操作符号(左引用), 不希望修改传进来的源对象

        T& operator[] (std::size_t p_index);  // 定义下标运算符

        const T& operator[] (std::size_t p_index) const;

        ~Array();

        T* Data() const;  // get m_data
 
        std::size_t Length() const;  // get m_length

        std::shared_ptr<T> DataHolder() const;  // get m_dataHolder

        void Set(T* p_array, std::size_t p_length, bool p_transferOwnership); // 外置数据覆盖
    ...
    // 已有外在p_dataHolder, 将所有权传递给m_dataHolder
    template<typename T>
    ...
    // 左值引用必须绑定:1.有名字 2. 有持久存储地址 3. 可以取地址的对象
    // 右值引用的一种情况, 函数体内部的临时变量作为返回值
    template<typename T>
    ...
    // 返回Array<T的引用, Array<T>::operator= 移动赋值运算符定义
    template<typename T>
    Array<T>&
    ...
    // 第一个const表示返回值无法被修改， 第二个const表示函数不修改对象，让const对象也能用这个方法
    template<typename T>
    const T&
    ...
    //	函数尾部的 const 只是保证这个成员函数不会修改当前对象的成员变量（比如 m_data、m_length）， 但并不限制通过返回值去修改数据内容。
    ...
    Array<T>::Alloc(std::size_t p_length)
    {
        ...
        arr.m_data = arr.m_dataHolder.get(); // m_data 指向 shared_ptr 管理的数组的起始位置（原始指针）
        return arr;
    }


    typedef Array<std::uint8_t> ByteArray; // std::uint8_t 无符号8位整数，0-255，一个字节
}
