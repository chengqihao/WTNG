...
namespace stkq
{
    namespace
    {
        void SaveHnswGraph(Index *index, std::ostream &out)
        {
            unsigned enterpoint_id = index->enterpoint_->GetId();
            unsigned max_level = index->max_level_;
            ...
            {
                unsigned node_id = index->nodes_[i]->GetId();
                out.write((char *)&node_id, sizeof(unsigned));
                unsigned node_level = index->nodes_[i]->GetLevel() + 1;
                out.write((char *)&node_level, sizeof(unsigned));
                for (unsigned j = 0; j < node_level; j++)
                {
                    unsigned current_level_GK = index->nodes_[i]->GetFriends(j).size();
                    out.write((char *)&current_level_GK, sizeof(unsigned));
                    for (unsigned k = 0; k < current_level_GK; k++)
                    {
                        unsigned current_level_neighbor_id = index->nodes_[i]->GetFriends(j)[k]->GetId();
            ...
            for (unsigned i = 0; i < index->getBaseLen(); i++)
            {
                ...
                for (unsigned j = 0; j < node_level; j++)
                {
                    ...
                    for (unsigned k = 0; k < current_level_GK; k++)
                    {
                        ...
                        tmp.push_back(index->nodes_[current_level_neighbor_id]);
                    }
                    index->nodes_[node_id]->SetFriends(j, tmp);
                }
            }
            std::cout << "average_neighbor_size: " << average_neighbor_size / index->getBaseLen() << std::endl;
            index->enterpoint_ = index->nodes_[enterpoint_id];
        }

        void LoadHnswGraph(Index *index, const std::string &graph_file)
        {
            ...
            LoadHnswGraph(index, in);
            in.close();
        }

        void ShareLoadedData(Index *source, Index *target)
        {
            target->setBaseEmbData(source->getBaseEmbData());
            target->setBaseLocData(source->getBaseLocData());
            target->setBaseVideoData(source->getBaseVideoData());
            target->setQueryEmbData(source->getQueryEmbData());
            target->setQueryLocData(source->getQueryLocData());
            target->setQueryVideoData(source->getQueryVideoData());
            target->setQueryWeightData(source->getQueryWeightData());
            target->setGroundData(source->getGroundData());

            target->setBaseLen(source->getBaseLen());
            target->setQueryLen(source->getQueryLen());
            target->setGroundLen(source->getGroundLen());
            target->setBaseEmbDim(source->getBaseEmbDim());
            target->setBaseLocDim(source->getBaseLocDim());
            target->setBaseVideoDim(source->getBaseVideoDim());
            target->setQueryEmbDim(source->getQueryEmbDim());
            target->setQueryLocDim(source->getQueryLocDim());
            target->setQueryVideoDim(source->getQueryVideoDim());
            target->setGroundDim(source->getGroundDim());
    ...
    IndexBuilder *IndexBuilder::load(char *data_emb_file, char *data_loc_file, char *data_video_file, char *query_emb_file, char *query_loc_file, char *query_video_file, char *query_alpha_file, char *ground_file, Parameters &parameters, bool dual)
    {
        ...
        else
        {
            ...
            ShareLoadedData(final_index_1, final_index_2);
            final_index_2->set_alpha_1(1);
            final_index_2->set_alpha_2(0);
            ShareLoadedData(final_index_1, final_index_3);
            ...
            std::cout << "vbase loc alpha_1: " << final_index_1->get_alpha_1()
                      << " alpha_2: " << final_index_1->get_alpha_2() << std::endl;
            std::cout << "vbase emb alpha_1: " << final_index_2->get_alpha_1()
                      << " alpha_2: " << final_index_2->get_alpha_2() << std::endl;
            std::cout << "vbase video alpha_1: " << final_index_3->get_alpha_1()
                      << " alpha_2: " << final_index_3->get_alpha_2() << std::endl;
            std::cout << "=====================" << std::endl;
            return this;
        }
    }
    ...
    IndexBuilder *IndexBuilder::init(TYPE type, bool debug)
    {
        ...
        else if (type == INIT_VBASE)
        {
            std::cout << "__INIT : VBASE__" << std::endl;
            a = new ComponentInitVBASE(final_index_1, final_index_2, final_index_3);
        }
    ...
    IndexBuilder *IndexBuilder::save_graph(TYPE type, char *graph_file)
    {
        if (type == INDEX_VBASE)
        {
            std::string graph_prefix(graph_file);
            SaveHnswGraph(final_index_1, graph_prefix + "_1");
            SaveHnswGraph(final_index_2, graph_prefix + "_2");
            SaveHnswGraph(final_index_3, graph_prefix + "_3");
            return this;
        }
    ...
    IndexBuilder *IndexBuilder::load_graph(TYPE type, char *graph_file)
    {
        ...
        {
            std::string graph_prefix(graph_file);
            LoadHnswGraph(final_index_1, graph_prefix + "_1");
            LoadHnswGraph(final_index_2, graph_prefix + "_2");
            LoadHnswGraph(final_index_3, graph_prefix + "_3");
            return this;
        }
    ...
    IndexBuilder *IndexBuilder::search(TYPE entry_type, TYPE route_type, TYPE L_type, Parameters param_)
    {
        ...
        if (route_type == DUAL_ROUTER_HNSW)
        {
            ...
            if (L_type == L_SEARCH_ASCEND)
            {
                ...
                for (unsigned t = 0; t < 20; t++)
                {
                    ...
                    {
                        const float alpha_1 = final_index_1->getQueryWeightData()[static_cast<size_t>(i) * STRIDE + 0];
                        const float alpha_2 = final_index_1->getQueryWeightData()[static_cast<size_t>(i) * STRIDE + 1];
                        const float alpha_3 = 1.0f - alpha_1 - alpha_2;
                        ...
                        for (int j = 0; j < res_1[i].size(); j++)
                        {
                            ...
                            float d = alpha_1 * e_d + alpha_2 * s_d + alpha_3 * v_d;
        ...
        if (route_type == ROUTER_VBASE)
        {
            std::cout << "__SEARCH ENTRY : NONE__" << std::endl;
            std::cout << "__ROUTER : VBASE__" << std::endl;
            ComponentSearchRouteVBASE *b = new ComponentSearchRouteVBASE(final_index_1, final_index_2, final_index_3);
            std::vector<std::vector<unsigned>> res;
            ...
            if (L_type == L_SEARCH_ASCEND)
            {
                ...
                for (unsigned t = 0; t < 20; t++)
                {
                    ...
                    final_index_1->getParam().set<unsigned>("VBASE_K", K);
                    final_index_1->getParam().set<unsigned>("K_search", L);
                    final_index_2->getParam().set<unsigned>("K_search", L);
                    final_index_3->getParam().set<unsigned>("K_search", L);
                    final_index_1->getParam().set<unsigned>("L_search", L);
                    ...
                    auto s1 = std::chrono::high_resolution_clock::now();
                    unsigned long long total_stop_depth = 0;

                    res.clear();
                    res.resize(final_index_1->getQueryLen());
                    for (unsigned i = 0; i < final_index_1->getQueryLen(); i++)
                    {
                        std::vector<Index::Neighbor> pool;
                        b->RouteInner(i, pool, res[i]);
                        total_stop_depth += b->getLastStopDepth();
                    }

                    auto e1 = std::chrono::high_resolution_clock::now();
                    std::chrono::duration<double> diff = e1 - s1;
                    std::cout << "search time: " << diff.count() / final_index_1->getQueryLen() << "\n";
                    std::cout << "average stop depth: "
                              << static_cast<double>(total_stop_depth) / final_index_1->getQueryLen() << std::endl;
                    std::cout << "DistCount: "
                              << final_index_1->getDistCount() + final_index_2->getDistCount() + final_index_3->getDistCount()
                              << std::endl;
                    std::cout << "HopCount: "
                              << final_index_1->getHopCount() + final_index_2->getHopCount() + final_index_3->getHopCount()
                              << std::endl;
                    final_index_1->resetDistCount();
                    final_index_2->resetDistCount();
                    final_index_3->resetDistCount();
                    final_index_1->resetHopCount();
                    final_index_2->resetHopCount();
                    final_index_3->resetHopCount();
                    ...
                    for (unsigned i = 0; i < final_index_1->getQueryLen(); i++)
                    {
                        ...
                        for (unsigned j = 0; j < K; j++)
                        {
                            ...
                            for (; k < K; k++)
                            {
                                if (res[i][j] == final_index_1->getGroundData()[i * final_index_1->getGroundDim() + k])
                                    break;
                            }
                            if (k == K)
                                cnt++;
                    ...
                    float acc = recall / final_index_1->getQueryLen();
                    std::cout << K << " NN accuracy: " << acc << std::endl;
                }
            }
