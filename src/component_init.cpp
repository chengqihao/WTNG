...
#pragma omp for schedule(dynamic, 128) 
    ...
    void ComponentInitVBASE::InitInner()
    {
        std::cout << "__VBASE : BUILD LOC HNSW__" << std::endl;
        InitSingleModality(index, 0.0f, 1.0f);

        std::cout << "__VBASE : BUILD EMB HNSW__" << std::endl;
        InitSingleModality(emb_index, 1.0f, 0.0f);

        std::cout << "__VBASE : BUILD VIDEO HNSW__" << std::endl;
        InitSingleModality(video_index, 0.0f, 0.0f);
    }

    void ComponentInitVBASE::InitSingleModality(Index *target, float alpha_1, float alpha_2)
    {
        target->set_alpha_1(alpha_1);
        target->set_alpha_2(alpha_2);

        ComponentInitHNSW *builder = new ComponentInitHNSW(target);
        builder->InitInner();
    }
