...
namespace stkq
{
    ...
    class ComponentInitVBASE : public ComponentInit
    {
    public:
        ComponentInitVBASE(Index *loc_index, Index *emb_index, Index *video_index)
            : ComponentInit(loc_index), emb_index(emb_index), video_index(video_index) {}

        void InitInner() override;

    private:
        void InitSingleModality(Index *target, float alpha_1, float alpha_2);

        Index *emb_index = nullptr;
        Index *video_index = nullptr;
    };
    ...
    class ComponentSearchEntry;

    class ComponentSearchRouteVBASE : public ComponentSearchRoute
    {
    public:
        ComponentSearchRouteVBASE(Index *loc_index, Index *emb_index, Index *video_index);

        void RouteInner(unsigned query, std::vector<Index::Neighbor> &pool, std::vector<unsigned> &res) override;

        unsigned getLastStopDepth() const { return last_stop_depth; }

    private:
        Index *emb_index = nullptr;
        Index *video_index = nullptr;
        ComponentSearchEntry *loc_entry = nullptr;
        ComponentSearchEntry *emb_entry = nullptr;
        ComponentSearchEntry *video_entry = nullptr;
        ComponentSearchRoute *loc_route = nullptr;
        ComponentSearchRoute *emb_route = nullptr;
        ComponentSearchRoute *video_route = nullptr;
        unsigned last_stop_depth = 0;
    };
