...
#pragma omp for schedule(dynamic, 128)
    ...
    {
        for (unsigned j = 0; j < index->getBaseEmbDim(); j++)
            index->Discrete_emb_center[j] = 0; 
        for (unsigned i = 0; i < index->getBaseLen(); i++)
        {
            for (unsigned j = 0; j < index->getBaseEmbDim(); j++)
            {
                index->Discrete_emb_center[j] += index->getBaseEmbData()[static_cast<size_t>(i) * index->getBaseEmbDim() + j];
            }
        }
        for (unsigned j = 0; j < index->getBaseEmbDim(); j++)
        {
            index->Discrete_emb_center[j] /= index->getBaseLen();
        }

        for (unsigned j = 0; j < index->getBaseLocDim(); j++)
            index->Discrete_loc_center[j] = 0;

        for (unsigned i = 0; i < index->getBaseLen(); i++)
        {
            for (unsigned j = 0; j < index->getBaseLocDim(); j++)
            {
                index->Discrete_loc_center[j] += index->getBaseLocData()[static_cast<size_t>(i) * index->getBaseLocDim() + j];
            }
        }

        for (unsigned j = 0; j < index->getBaseLocDim(); j++)
        {
            index->Discrete_loc_center[j] /= index->getBaseLen();
        };
    }
    ...
    void ComponentInitWTNG::BuildByIncrementInsert()
    {
        ...
        index->Discrete_emb_center = new float[index->getBaseEmbDim()];  
        index->Discrete_loc_center = new float[index->getBaseLocDim()];
...
#pragma omp for schedule(dynamic, 128)
    ...
    void ComponentInitWTNG::findSkyline(std::vector<Index::WTNGNNDescentNeighbor> &points, std::vector<Index::WTNGNNDescentNeighbor> &skyline,
    ...
    {
        float e_d = index->get_E_Dist()->compare(index->getBaseEmbData() + (size_t)qnode->GetId() * index->getBaseEmbDim(),
                                                 index->Discrete_emb_center,
                                                 index->getBaseEmbDim());

        float s_d = index->get_S_Dist()->compare(index->getBaseLocData() + (size_t)qnode->GetId() * index->getBaseLocDim(),
                                                 index->Discrete_loc_center,
                                                 index->getBaseLocDim());

        {
            std::unique_lock<std::mutex> enterpoint_lock(index->Discrete_enterpoint_mutex);

            index->WTNG_enterpoints_skyeline.emplace_back(Index::WTNGNNDescentNeighbor(qnode->GetId(), e_d, s_d, true, 0));  // vector<DEGNNDescentNeighbor>

      
            sort(index->WTNG_enterpoints_skyeline.begin(), index->WTNG_enterpoints_skyeline.end());

            float max_emb_dis = 0;
            float min_emb_dis = 1e9; // 多余变量

            std::vector<Index::WTNGNNDescentNeighbor> skyline;
    ...
    void ComponentInitWTNG::InsertNode(Index::WTNGNode *qnode, Index::VisitedList *visited_list)
    {
        ...
        std::vector<Index::WTNGNNDescentNeighbor> pool;
                                         ...
                                         std::vector<Index::WTNGNNDescentNeighbor> &pool)
    {
        ...
        std::unique_lock<std::mutex> enterpoint_lock(index->Discrete_enterpoint_mutex, std::defer_lock);
        ...
        auto queue = Index::skyline_queue_my(ef_construction);
        queue.init_queue(pool);
    ...
    void ComponentInitWTNG::Link(Index::WTNGNode *source, Index::WTNGNode *target, int level, float e_dist, float s_dist)
    {
        std::unique_lock<std::mutex> lock(source->GetAccessGuard());
        std::vector<Index::WTNGNeighbor> &neighbors = source->GetFriends();
        std::vector<Index::WTNGNNDescentNeighbor> tempres;
        std::vector<Index::WTNGNeighbor> result;
        tempres.emplace_back(Index::WTNGNNDescentNeighbor(target->GetId(), e_dist, s_dist, true, -1));
        for (const auto &neighbor : neighbors)
        {
            tempres.emplace_back(Index::WTNGNNDescentNeighbor(neighbor.id_, neighbor.emb_distance_, neighbor.geo_distance_, true, -1));
        }
        neighbors.clear();
        ComponentWTNGPruneHeuristic *a = new ComponentWTNGPruneHeuristic(index);
        a->DEG2Neighbor(source->GetId(), source->GetMaxM(), tempres, result);
        source->SetFriends(result);
        std::vector<Index::WTNGNNDescentNeighbor>().swap(tempres);
